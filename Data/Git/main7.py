...
for i, frame in enumerate(ImageSequence.Iterator(gif)):
    ...
    # Определяем рамку вручную (по центру кадра)
    head_w, head_h = int(w*0.55), int(h*0.75)  # размеры рамки (55% ширины, 75% высоты)
    x1 = (w - head_w) // 2
    y1 = (h - head_h) // 2
    x2 = x1 + head_w
    y2 = y1 + head_h
    ...
    # Засечки (по 3 на каждую сторону, центральные длиннее внутрь)
